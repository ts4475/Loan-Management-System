...
const Tracking3 = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col items-center bg-[#E7EEE1]">
            <Navbar/>
            <br/>
            <h2 className="text-2xl font-bold mb-6">Tracking 3 Page</h2>
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg w-1/2">
                    ...
                    {documents.map(doc => (
                        <div key={doc.document_id} className="mt-2">
                            ...
                            <a href={doc.document_url} target="_blank" rel="noopener noreferrer" className="text-blue-500">View</a>
                            <a href={doc.document_url} download className="ml-2 text-blue-500">Download</a>
                            {['pdf', 'jpg', 'jpeg', 'png'].includes(doc.document_url.split('.').pop().toLowerCase()) && (
                                <div className="mt-2">
                                    {doc.document_url.split('.').pop().toLowerCase() === 'pdf' ? (
                                        <embed src={doc.document_url} width="200" height="200" type="application/pdf" />
                                    ) : (
                                        <img src={doc.document_url} alt={doc.document_type} className="w-40 h-40 object-cover" />
                                    )}
                                </div>
                            )}
                        </div>
                    ))}
                    <input type="radio" checked readOnly className="text-green-500 mt-4" />
                    <p>Document Verification Pending</p>
                    <input type="radio" checked readOnly className="text-yellow-500 mt-2" />
                </div>
            </div>
            <div className="flex justify-between w-full max-w-6xl">
                <button onClick={handleGoBack} className="bg-gray-500 text-white py-2 px-4 rounded">Go Back to Dashboard</button>
                <button onClick={handleCancel} className="bg-red-500 text-white py-2 px-4 rounded ml-4">Cancel Application</button>
            </div>
        </div>
    );
};
